...
# We could instead have two Stacks s inside our MinStack. The main Stack should keep track of the order numbers 
...
# The push(...) method for this implementation of MinStack is straightforward. Items should always be pushed 
# onto the main Stack, but they should only be pushed onto the min-tracker Stack if they are smaller than the 
# current top of it. 
...
# MinStack's pop(...) method. The value we actually need to pop is always on the top of the main underlying 
# Stack. 
# However, if we simply popped it from there, the min-tracker Stack would become incorrect once its top value 
# had been removed from the main Stack.

# A logical solution would be to do the following additional check and modification to the min-tracker Stack 
# when MinStack's pop(...) method is called.
...
# the condition for popping the min-tracker Stack too was triggered (i.e. that both internal stacks have the 
# same top). This isn't what we wanted though—it was the earlier 6 that triggered the push(...) onto the 
# min-tracker Stack, not the latter one! The 6 should have been left alone with that first pop(...).
