...
# purpose, it supports fast look up in near constant time. 

# A simple implementation uses two iterations. In the first iteration, we add each element's value and its index to 
# the table. Then, in the second iteration we check if each element's complement (target−nums[i]) exists in the table. 
# Beware that the complement must not be nums[i] itself!


# Complexity Analysis:
# Time complexity : O(n). We traverse the list containing nn elements exactly twice. Since the hash table reduces 
# the look up time to O(1), the time complexity is O(n).
