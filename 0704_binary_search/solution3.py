...
# we look for the insert position where we can put target in without disrupting the order.
...
# - If nums[mid] < target, the insert position is on mid's right, so we let left = mid + 1 to discad the left half 
#   and mid.
# - If nums[mid] = target, the insert position is on mid's right, so we let left = mid + 1 to discad the left half 
#   and mid.
...
# Algorithm:
# - Initialize the boundaries of the search space as left = 0 and right = nums.size (Note that the maximum insert 
#   position can be nums.size)
# - If there are elements in the range [left, right], we find the middle index mid = (left + right) / 2 and compare 
#   the middle value nums[mid] with target:
